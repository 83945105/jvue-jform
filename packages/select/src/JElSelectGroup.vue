  ...
  export default {
    ...
    props: {
      ...
      dataProp: {                         // 数据属性
        ...
        default() {
          return {
            groupLabelKey: 'label',         // 用于解析分组标签的key
            groupDisabledKey: 'disabled',   // 用于解析分组是否禁用的key
            groupDataKey: 'data',           // 用于解析分组数据的key
      ...
      data: {
        ...
        handler(val) {
          this.data_ = (val || []).map(group => {
            return {
              label: group[this.dataProp.groupLabelKey],
              disabled: !!group[this.dataProp.groupDisabledKey],
              data: (group[this.dataProp.groupDataKey] || []).map(option => {
                return {
